...
class AppNavbar extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <Navbar expand="sm" className="mb-4" dark>
          ...
          <Collapse isOpen={this.state.isOpen} navbar>
            <Nav className="ml-auto" navbar>
              <NavItem>
                <Dropdown
                  isOpen={this.state.botStatusDropdownOpen}
                  toggle={this.toggleBotStatusDropdown}
                >
                  <DropdownToggle caret className="bg-transparent">
                    Bot Status:{" "}
                    {status ? (
                      <span>
                        <span className="status-icon bg-success img-circle" />
                        Online
                      </span>
                    ) : (
                      <span>
                        <span className="status-icon bg-danger img-circle" />
                        Offline
                      </span>
                    )}
                  </DropdownToggle>
                  <DropdownMenu>
                    ...
                    {status ? (
                      <DropdownItem onClick={this.props.turnBotOff}>
                        Turn bot off
                      </DropdownItem>
                    ) : (
                      <DropdownItem onClick={this.props.turnBotOn}>
                        Turn bot on
                      </DropdownItem>
                    )}
                  </DropdownMenu>
...
export default connect(
  mapStateToProps,
  { getBotStatus, turnBotOff, turnBotOn }
)(AppNavbar);
