...
function sellFirebase(market, profitLoss, sellTime) {
  ...
  var newSellKey = firebaseDb
    .ref()
    .child("sells")
    .push().key;
  // get openTrades and iterate to find the market to sell
  openTrades.once("value").then(data => {
    data.forEach(trade => {
...
// @desc    Get pricing data for ticker using ccxt lib
...
router.post("/sell", (req, res) => {
  ...
  (async () => {
    if (price == undefined) {
      ...
      try {
        const tickerData = await exchange.fetchTicker(market); // get current pricing
        try {
          // attempte limit sell order
          const response = await exchange.createLimitSellOrder(
            market,
            amount,
            tickerData.ask
          );
      ...
      // we have a defined price to sell at
      try {
        const response = await exchange.createLimitSellOrder(
          market,
          amount,
